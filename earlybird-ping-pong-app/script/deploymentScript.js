...
const deployEarlybirdPingPongApp = async () => {
  try {
    ...
    let expectedEarlybirdPingPongAppData = await readExpectedEarlybirdPingPongAppData(EARLYBIRD_PING_PONG_APP_DATA_FILE_PATH);
...
async function useOrDeployThunderbirdPingPongApp(
    ...
    if ((expectedThunderbirdPingPongAppCode == '0x') && (earlybirdEndpointCode != '0x')) {
        const thunderbirdPingPongAppFactory = new ethers.ContractFactory(thunderbirdPingPongAppData.abi, thunderbirdPingPongAppData.bytecode, wallet);
...
async function useOrDeployRukhPingPongApp(
    ...
    if ((expectedRukhPingPongAppCode == '0x') && (earlybirdEndpointCode != '0x')) {
        const rukhPingPongAppFactory = new ethers.ContractFactory(rukhPingPongAppData.abi, rukhPingPongAppData.bytecode, wallet);
