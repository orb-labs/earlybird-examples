...
import "../../../src/RukhVersion/SendingRelayer.sol";
...
contract SendingRelayerDeployment is Script {
    function run() external {
        ...
        // expected address
        address expectedRukhVersionPingPongSendingRelayerAddress = vm.envAddress(
        ...
        assembly {
            size := extcodesize(expectedRukhVersionPingPongSendingRelayerAddress)
        }
        ...
        if (size == 0) {
            ...
            console.log("Rukh Version PingPong SendingRelayer deployed on %s", chainName);
        } else {
            console.log("Rukh Version PingPong SendingRelayer already deployed on %s", chainName);
        }
    }
}
